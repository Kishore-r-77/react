...
import DeathClaimFundDetailsPasAdd from "./DeathClaimFundDetailsPasAdd";
import DeathClaimFundDetailsPasEdit from "./DeathClaimFundDetailsPasEdit";
import DeathClaimFundDetailsPasInfo from "./DeathClaimFundDetailsPasInfo";
...
function DeathClaimFundDetailsPas() {
  ...
  const getData = () => {
    axios
      .get(`http://localhost:8080/deathClaimFundDetails/getall/${userId}`, {
        headers: {
  ...
  const handleFormSubmit = () => {
    axios
      .post(
        `http://localhost:8080/deathClaimFundDetails/add/${userId}`,
        {
          companyId: fundDetailsPasData.companyId,
          policyNum: fundDetailsPasData.policyNum,
          uinNumber: fundDetailsPasData.uinNumber,
  ...
  const editFormSubmit = () => {
    axios
      .patch(
        `http://localhost:8080/deathClaimFundDetails/update/${record.id}/${userId}`,
        {
          companyId: record.companyId,
          policyNum: record.policyNum,
          uinNumber: record.uinNumber,
          fundCode: record.fundCode,
          fundName: record.fundName,
          navDate: moment(record.navDate)
            .format("YYYYMMDD")
            .toString(),
          units: record.units,
  ...
  const handleDelete = (id) => {
    ...
    axios
      .patch(
        `http://localhost:8080/deathClaimFundDetails/softdelete/${id}/${userId}`,
        {},
        {
          headers: {
            Authorization: `Bearer ${sessionStorage.getItem("token")}`,
  ...
  const globalsearch = (val) => {
    val === ""
      ? getData()
      : axios
          .get(`http://localhost:8080/deathClaimFundDetails/search/${val}`, {
            headers: {
              Authorization: `Bearer ${sessionStorage.getItem("token")}`,
            },
          })
  ...
  return (
    <>
      <div className="classTitle">
        <h2>
          {" "}
          <b>DeathClaim Fund Details PAS</b>{" "}
        </h2>
      </div>
...
export default DeathClaimFundDetailsPas;
