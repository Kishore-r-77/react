...
import DeathClaimLeapFundDetailsEdit from "./DeathClaimLeapFundDetailsEdit";
import DeathClaimLeapFundDetailsInfo from "./DeathClaimLeapFundDetailsInfo";
...
function DeathClaimLeapFundDetails() {
  const access = JSON.parse(sessionStorage.getItem("specialaccess"));
  const userId = sessionStorage.getItem("userId");

  const [data, setData] = useState([]);
  const [purpleFundDetailsData, setpurpleFundDetailsData] = useState(
    initialValues
  );
  ...
  const getData = () => {
    axios
      .get(`http://localhost:8080/deathClaimLeapFundDetails/getall/${userId}`, {
        headers: {
  ...
  const globalsearch = (val) => {
    val === ""
      ...
      : axios
          .get(
            `http://localhost:8080/deathClaimLeapFundDetails/search/${val}`,
            {
              headers: {
                Authorization: `Bearer ${sessionStorage.getItem("token")}`,
              },
  ...
  const editFormSubmit = () => {
    axios
      .patch(
        `http://localhost:8080/deathClaimLeapFundDetails/update/${record.id}/${userId}`,
        {
  ...
  const handleDelete = (id) => {
    ...
    axios
      .patch(
        `http://localhost:8080/deathClaimLeapFundDetails/softdelete/${id}/${userId}`,
        {},
        {
          headers: {
            Authorization: `Bearer ${sessionStorage.getItem("token")}`,
...
export default DeathClaimLeapFundDetails;
