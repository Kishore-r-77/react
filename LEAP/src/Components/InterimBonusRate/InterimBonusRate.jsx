...
import InterimBonusRateAdd from "./InterimBonusRateAdd";
import InterimBonusRateEdit from "./InterimBonusRateEdit";
import InterimBonusRateInfo from "./InterimBonusRateInfo";
...
function InterimBonusRate() {
  const access = JSON.parse(sessionStorage.getItem("specialaccess"));
  const userId = sessionStorage.getItem("userId");

  const [data, setData] = useState([]);
  const [interimbonusRateData, setinterimbonusRateData] = useState(
    initialValues
  );
  ...
  const onChange = (e) => {
    ...
    setinterimbonusRateData({ ...interimbonusRateData, [name]: value });
  };

  // Customer Details On Change Date

  const onChangeFinancialYear = (date) => {
    setinterimbonusRateData({ ...interimbonusRateData, financialYear: date });
  };
  const onChangeStartDate = (date) => {
    setinterimbonusRateData({ ...interimbonusRateData, startDate: date });
  };
  const onChangeEndDate = (date) => {
    setinterimbonusRateData({ ...interimbonusRateData, endDate: date });
  };
  ...
  const getData = () => {
    axios
      .get(`http://localhost:8080/interimBonusRate/getall/${userId}`, {
        headers: {
  ...
  const handleFormSubmit = () => {
    axios
      .post(
        `http://localhost:8080/interimBonusRate/add/${userId}`,
        {
          companyId: interimbonusRateData.companyId,
          uinNumber: interimbonusRateData.uinNumber,
          planName: interimbonusRateData.planName,
          planCode: interimbonusRateData.planCode,
          bonusRate: interimbonusRateData.bonusRate,
          bonusType: interimbonusRateData.bonusType,
          financialYear: moment(interimbonusRateData.financialYear)
            .format("YYYYMMDD")
            .toString(),
          startDate: moment(interimbonusRateData.startDate)
            .format("YYYYMMDD")
            .toString(),
          endDate: moment(interimbonusRateData.endDate)
            .format("YYYYMMDD")
            .toString(),
          createdBy: userId,
          modifiedBy: userId,
      ...
      .then((resp) => {
        console.log(resp);
        handleClickClose();
        setinterimbonusRateData(initialValues);
        getData();
        setNotify({
          isOpen: true,
          message: resp.data,
  ...
  const editFormSubmit = () => {
    axios
      .patch(
        `http://localhost:8080/interimBonusRate/update/${record.id}/${userId}`,
        {
  ...
  const handleDelete = (id) => {
    ...
    axios
      .patch(
        `http://localhost:8080/interimBonusRate/softdelete/${id}/${userId}`,
        {},
        {
          headers: {
            Authorization: `Bearer ${sessionStorage.getItem("token")}`,
  ...
  const globalsearch = (val) => {
    val === ""
      ? getData()
      : axios
          .get(`http://localhost:8080/interimBonusRate/search/${val}`, {
            headers: {
              Authorization: `Bearer ${sessionStorage.getItem("token")}`,
            },
          })
  ...
  const upload = (e) => {
    ...
    axios.post(`http://localhost:8080/interimBonusRate/upload`, formData, {
      headers: {
...
export default InterimBonusRate;
