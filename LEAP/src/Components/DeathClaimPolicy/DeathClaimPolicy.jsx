...
import DeathClaimPolicyAdd from "./DeathClaimPolicyAdd";
import DeathClaimPolicyEdit from "./DeathClaimPolicyEdit";
import DeathClaimPolicyInfo from "./DeathClaimPolicyInfo";
import DeathClaimPolicyCover from "./DeathClaimPolicyCover";
...
function DeathClaimPolicy() {
  const access = JSON.parse(sessionStorage.getItem("specialaccess"));
  const userId = sessionStorage.getItem("userId");

  const [data, setData] = useState([]);
  const [deathClaimPolicy, setdeathClaimPolicy] = useState([]);
  const [deathClaimPolicyData, setdeathClaimPolicyData] = useState(
    initialValues
  );
  ...
  const onChange = (e) => {
    ...
    setdeathClaimPolicyData({
      ...deathClaimPolicyData,
      [name]: value,
    });
  };
  ...
  //   setdeathClaimPolicyData({ ...deathClaimPolicyData, fup: date });
  ...
  const getData = () => {
    axios
      .get(`http://localhost:8080/deathClaimPolicyDetails/getall/${userId}`, {
        headers: {
  ...
  const getCoverDetails = (policyId) => {
    axios
      .get(
        `http://localhost:8080/deathClaimCoverPas/getByPolicyNo/${policyId}`,
        {
          headers: {
            Authorization: `Bearer ${sessionStorage.getItem("token")}`,
          },
  ...
  const handleFormSubmit = () => {
    axios
      .post(
        `http://localhost:8080/deathClaimPolicyDetails/add/${userId}`,
        {
          companyId: deathClaimPolicyData.companyId,
          clntNum: deathClaimPolicyData.clntNum,
          chdrNum: deathClaimPolicyData.chdrNum,
          billFreq: deathClaimPolicyData.billFreq,
          installmentPremium: deathClaimPolicyData.installmentPremium,
          extraPremium: deathClaimPolicyData.extraPremium,
          uinNumber: deathClaimPolicyData.uinNumber,
          fup: moment(deathClaimPolicyData.fup)
            .format("YYYYMMDD")
            .toString(),
          docDate: moment(deathClaimPolicyData.docDate)
            .format("YYYYMMDD")
            .toString(),
          laAge: deathClaimPolicyData.laAge,
          phAge: deathClaimPolicyData.phAge,
          statusCode: deathClaimPolicyData.statusCode,
          smokerFlag: deathClaimPolicyData.smokerFlag,
          createdBy: userId,
          modifiedBy: userId,
        },
        {
      ...
      .then((resp) => {
        console.log(resp);
        handleClickClose();
        setdeathClaimPolicyData(initialValues);
        getData();
        setNotify({
          isOpen: true,
          message: resp.data,
  ...
  const editFormSubmit = () => {
    axios
      .patch(
        `http://localhost:8080/deathClaimPolicyDetails/update/${record.id}/${userId}`,
        {
  ...
  const handleDelete = (id) => {
    ...
    axios
      .patch(
        `http://localhost:8080/deathClaimPolicyDetails/softdelete/${id}/${userId}`,
        {},
        {
          headers: {
            Authorization: `Bearer ${sessionStorage.getItem("token")}`,
  ...
  const globalsearch = (val) => {
    val === ""
      ? getData()
      : axios
          .get(`http://localhost:8080/deathClaimPolicyDetails/search/${val}`, {
            headers: {
              Authorization: `Bearer ${sessionStorage.getItem("token")}`,
            },
          })
  ...
  return (
    <>
      <div className="classTitle">
        <h2>
          {" "}
          <b>Death Claim Policy Details</b>{" "}
        </h2>
      </div>
            ...
            {data
              ...
              .map((value, index) => (
                <tr>
                  ...
                  <td>{moment(value.docDate).format("DD-MM-YYYY")}</td>
                  <td>{moment(value.fup).format("DD-MM-YYYY")}</td>
      ...
      <DeathClaimPolicyEdit
        ...
        smokerFlagData={smokerFlagData}
        onChange={editChange}
        onChangeFupDate={editChangeFup}
        onChangedocDate={editChangedocDate}
        handleFormSubmit={() => editFormSubmit()}
      />
      <DeathClaimPolicyInfo
        open={infoOpen}
        handleClose={handleInfoClose}
        data={info}
      />

      <DeathClaimPolicyCover
        open={coverOpen}
        close={coverclose}
        policyCovers={policyCovers}
        coverDetails={coverDetails}
        setpolicyCovers={setpolicyCovers}
        policyDetails={deathClaimPolicy}
        setpolicyDetails={setdeathClaimPolicy}
        getAll={getData}
        chdrId={chdrId}
        cmpId={cmpId}
        clientId={clientId}
...
export default DeathClaimPolicy;
