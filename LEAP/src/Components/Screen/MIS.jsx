...
function MIS() {
  ...
  function downloadPdf(val) {
    axios({
      url: `http://localhost:8080/purpledetails/generatePdf/${val}`,
      method: "GET",
      responseType: "blob",
      headers: {
        Authorization: `Bearer ${sessionStorage.getItem("token")}`,
      ...
      const link = document.createElement("a");
      link.href = url;
      link.setAttribute("download", "LEAPS.pdf");
      link.click();
    });
  }
  ...
  const handleDelete = (val) => {
    ...
    axios
      .patch(
        `http://localhost:8080/purpledetails/reinitiateTrans/${val}`,
        {},
        {
          headers: {
            Authorization: `Bearer ${sessionStorage.getItem("token")}`,
              ...
              .map((value, index) => (
                <tr>
                  ...
                  {search === "Passed" || search === "Failed" ? (
                    <td>
                      <DownloadIcon
                        onClick={() => downloadPdf(value.policyNo)}
                      />
                    </td>
                  ) : null}
                  {search !== "Initiated" ? (
