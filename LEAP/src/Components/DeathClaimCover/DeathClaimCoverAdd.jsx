...
function DeathClaimCoverAdd({
  ...
  const handleFormSubmit = () => {
    ...
    axios
      .post(`http://localhost:8080/deathClaimCoverPas/add/${userId}`, data, {
        headers: {
          Authorization: `Bearer ${sessionStorage.getItem("token")}`,
        },
      })
      .then((resp) => {
        if (open1 === true) {
          ...
          window.location = "deathClaimPolicy";
        } else {
          handleClickClose();
          getdata();
          setNotify({
...
export default DeathClaimCoverAdd;
